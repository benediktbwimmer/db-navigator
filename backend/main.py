...
from backend.db_tools import get_db_schema, execute_query, get_db_connection, POSTGRES_CONN_STRING
...
# --- Helper Function ---
async def get_table_row_count(table: str, conn_str: str | None = None) -> int:
...
# --- Endpoints ---
@app.get("/api/connection-string")
async def get_default_connection():
    """Return the default connection string from the environment"""
    return {"connection_string": POSTGRES_CONN_STRING}

@app.post("/api/schema", response_model=SchemaResponse)
async def fetch_schema(request: SchemaRequest):
    try:
        schema_obj = await get_db_schema(request.connection_string)
        row_counts = {}
        for table in schema_obj["tables"].keys():
            row_counts[table] = await get_table_row_count(table, request.connection_string)
...
async def execute_sql(request: ExecuteSQLRequest):
    try:
        ...
        query_result = await execute_query(request.query, request.connection_string)
